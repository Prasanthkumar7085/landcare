...
export class MapsController {
  async addMap(reqData: IMap, res: NextResponse) {
    try {
      ...
      const existedSlugMap = await mapsDataServiceProvider.findMapBySlug(
        reqData.slug
      );
      if (existedSlugMap) {
  ...
  async updateOne(reqData: any, params: any) {
    try {
      ...
      const existedMap: any = await mapsDataServiceProvider.findMapByTitleAndId(
        normalizedTitle,
        params.id
      );
      if (existedMap) {
      ...
      const existedSlug = await mapsDataServiceProvider.findMapBySlugAndId(
        slug,
        params.id
      );
      if (existedSlug) {
  ...
  async getMapBySlug(params: any) {
    try {
      const mapData: any = await mapsDataServiceProvider.findMapBySlug(
        params.slug
      );
      if (!mapData) {
        return ResponseHelper.sendErrorResponse(400, MAP_NOT_FOUND);
